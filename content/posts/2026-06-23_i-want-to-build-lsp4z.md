+++
title = "LSP4Z をつくりたい（Zed で言語サーバを動かすための選択肢について）"
date = 2026-06-23
...
最近は Zed を好んで使っている。エディタを使っているときの常だが、手元にある任意の言語サーバを動かしてエディタと連携させたくなることがある。設定ファイルをゴニョゴニョするだけで好きな言語サーバを使える体験を Zed で実現できないかと思い少し調査したので、それについてまとめておく。

## Zed で言語サーバを使う正規ルート

Zed における言語サポートは拡張機能によって提供される（[Language Extensions](https://zed.dev/docs/extensions/languages)）。拡張機能で言語を宣言して tree-sitter grammar と query を置いておくと、Zed はそれを利用してシンタックスハイライト・括弧の対応・自動インデントといったエディタとしての基本機能を実現する。
